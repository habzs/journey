...
const EditOpportunity: React.FC<EditOpportunityProps> = ({ params }) => {
  ...
  const updateOpportunity = async (
    ...
    if (role === "Admin") {
      try {
        ...
        // Update the badges in the subcollection - delete and add
        ...
        values.badges?.forEach((badge) => {
          const newBadgeRef = doc(badgesCollectionRef, badge.id);
          batch.set(newBadgeRef, badge);
        });
        await batch.commit();
        console.log("Badges updated successfully");

        // If status is "completed", add the badges to signed-up volunteers
        if (values.status === "completed") {
          ...
          const userBadgeUpdates = volunteersSnapshot.docs.map(
            async (volunteerDoc) => {
              ...
              // Update opportunity id and volunteer status in opportunities field
              const userDoc = await getDoc(userRef);
              if (userDoc.exists()) {
                const userData = userDoc.data();
                const opportunities = userData.opportunities || [];

                const updatedOpportunities = opportunities.map((opportunity: any) => {
                  if (opportunity[opportunityId]) {
                    return { [opportunityId]: "completed" };
                  }
                  return opportunity;
                });
  ...
  if (!role) {
    return <p>Loading...</p>; // Show loading message while role is being determined
  }
    ...
    <div className="w-full space-y-8">
      ...
      <div className="w-full flex flex-col space-y-5 md:flex-row md:space-x-5 md:space-y-0">
        ...
        <OpportunityDetailsCard
          opportunityId={opportunityId}
          formik={formik}
          role={role} // Pass the role as a prop
        />
      </div>
    </div>
  );
